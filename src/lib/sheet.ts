...
export const messages = {
  zh: {
    ...
    infoDescription:
      '利弊分析法适合在面对两难选择时，把“做”和“不做”分别拆成收益与代价，帮助你把模糊判断变成可比较的清单。',
    infoDetails: [
...
export function buildMarkdown(sheet: SheetState, copy: (typeof messages)[Locale]) {
  ...
  return [
    ...
    ...(doingPros.length
      ? doingPros.map((item) => `- ${item.bold ? `**${item.text}**` : item.text}`)
      : ['-']),
    '',
    `## ${copy.doing} / ${copy.cons}`,
    ...(doingCons.length
      ? doingCons.map((item) => `- ${item.bold ? `**${item.text}**` : item.text}`)
      : ['-']),
    '',
    `## ${copy.notDoing} / ${copy.pros}`,
    ...(notDoingPros.length
      ? notDoingPros.map((item) => `- ${item.bold ? `**${item.text}**` : item.text}`)
      : ['-']),
    '',
    `## ${copy.notDoing} / ${copy.cons}`,
    ...(notDoingCons.length
      ? notDoingCons.map((item) => `- ${item.bold ? `**${item.text}**` : item.text}`)
      : ['-']),
  ].join('\n')
}
